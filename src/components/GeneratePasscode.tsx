...
// "min-h-screen w-full bg-gradient-to-br from-gray-50 to-gray-100 px-4 py-12 sm:px-8">
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-tr from-white to-gray-100 px-4 py-10 sm:px-10">
  <div className=" max-w-3xl w-full bg-white p-8 rounded-xl shadow-lg border border-red-200">   
  {/* <div className="max-w-3xl mx-auto"> */}
    <div className="mb-8 text-center">
      <h1 className="text-3xl font-bold text-gray-900 mb-2">Passcode Generator</h1>
      <p className="text-gray-600 text-base">Generate and share secure passcodes instantly via WhatsApp.</p>
    </div>

    <div className="grid gap-8 sm:grid-cols-2 border border-gray-200 rounded-lg shadow-sm "> 
      <div className="bg-white p-6 rounded-xl ">
        ...
        <div className="mt-6 space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Custom Expiry Time</label>
            <input
              type="text"
            ...
            {expiresAt && !customExpiresAt && (
              <p className="text-xs text-gray-500 mt-2">Auto Expiry: {expiresAt.toLocaleTimeString()}</p>
            )}
          </div>
        </div>
      ...
      {name && passcode && (customExpiresAt || expiresAt) && (
        <div className="flex flex-col items-center justify-center">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center">
            <p className="text-sm text-gray-600 mb-4 font-medium">Scan QR to send via WhatsApp</p>
            <div className="p-4 bg-white rounded-lg border border-gray-200">
              <QRCode value={generateWhatsAppURL()} size={180} />
            </div>
            {/* <p className="text-xs text-gray-500 mt-4 text-center">The passcode will expire at {customExpiresAt || expiresAt.toLocaleTimeString()}</p> */}
          </div>
        </div>
      )}
    </div>
